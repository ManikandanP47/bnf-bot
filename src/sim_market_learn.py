...
def build_learning_snapshot(scan_result: dict) -> dict:
    ...
    return {
        ...
        'metrics': {
            ...
            'pcr': pcr,
            'max_pain': max_pain,
            'pro_checklist': (scan_result.get('pro_decision') or {}).get('checklist', {}),
            'pro_ranges': len((scan_result.get('pro_decision') or {}).get('ranges', {}).get('levels', [])),
            'chain_rr': ((scan_result.get('pro_decision') or {}).get('pick') or {}).get('rr_label'),
            'archetype': ((scan_result.get('pro_decision') or {}).get('pick') or {}).get('archetype'),
            'ce_pe_edge': ((scan_result.get('pro_decision') or {}).get('ce_pe') or {}).get('edge_note', '')[:80],
